                            ...
                            <section id="new">
                                ...
                                <form action="pages/function/Peminjaman.php?aksi=pinjam" method="POST">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="kategoriSelect" name="kategori">
                                            <option selected disabled value=""> -- Silahkan pilih kategori buku -- </option>
                                            <?php
                                            include "../../config/koneksi.php";

                                            $sql = mysqli_query($koneksi, "SELECT DISTINCT kategori_buku FROM buku ORDER BY kategori_buku ASC");
                                            while ($data = mysqli_fetch_array($sql)) {
                                            ?>
                                                <option value="<?= $data['kategori_buku']; ?>"> <?= $data['kategori_buku']; ?></option>
                                            <?php
                                            }
                                            ?>
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="judulBukuSelect" name="judulBuku">
                                            ...
                                            while ($data = mysqli_fetch_array($sql)) {
                                            ?>
                                                <option value="<?= $data['judul_buku']; ?>" data-kategori="<?= $data['kategori_buku']; ?>"> <?= $data['judul_buku']; ?></option>
                                            <?php
                                            }
                                            ?>
                                        </select>
...
<script>
    $(document).ready(function() {
        $('#kategoriSelect').on('change', function() {
            ...
            const judulBukuSelect = $('#judulBukuSelect');
            
            // Reset judul buku dropdown
            judulBukuSelect.find('option').each(function() {
                const kategori = $(this).data('kategori');
                if ($(this).val() === '') {
                    $(this).show();
                } else if (selectedKategori === '' || kategori === selectedKategori) {
                    $(this).show();
                } else {
                    $(this).hide();
                }
